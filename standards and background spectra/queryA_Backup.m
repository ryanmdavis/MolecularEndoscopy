%call: [A,wavenumber,channel_names,num_np_channels,A_info]=queryA_Backup(rp.A_info);

function varargout = queryA_Backup(varargin)
% QUERYA_BACKUP MATLAB code for queryA_Backup.fig
%      QUERYA_BACKUP by itself, creates a new QUERYA_BACKUP or raises the
%      existing singleton*.
%
%      H = QUERYA_BACKUP returns the handle to a new QUERYA_BACKUP or the handle to
%      the existing singleton*.
%
%      QUERYA_BACKUP('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QUERYA_BACKUP.M with the given input arguments.
%
%      QUERYA_BACKUP('Property','Value',...) creates a new QUERYA_BACKUP or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before queryA_Backup_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to queryA_Backup_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help queryA_Backup

% Last Modified by GUIDE v2.5 22-Nov-2017 14:51:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @queryA_Backup_OpeningFcn, ...
                   'gui_OutputFcn',  @queryA_Backup_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before queryA_Backup is made visible.
function queryA_Backup_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to queryA_Backup (see VARARGIN)

% Choose default command line output for queryA_Backup
handles.output = 'Yes';
...
prdata.defaults=readModelSpectra('defaults');
prdata.endoscope_spectra=readModelSpectra('endoscope_sers');
prdata.microscope_spectra=readModelSpectra('microscope_sers');
prdata.endoscope_background=readModelSpectra('endoscope_background');
...
createA(handles);
% UIWAIT makes queryA_Backup wait for user response (see UIRESUME)
uiwait(handles.figure1);

% --- Outputs from this function are returned to the command line.
function varargout = queryA_Backup_OutputFcn(hObject, eventdata, handles)
...
if get(handles.popupmenu1,'value')
    out=readModelSpectra('endoscope_sers');
else
    out=readModelSpectra('microscope_sers');
end
...
if get(handles.popupmenu1,'value')==1 %endoscope
    ...
    non_default_channels=cellSetDifference({prdata.microscope_spectra.spectra.name},prdata.defaults.endoscope_channels);
